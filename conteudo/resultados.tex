...
\done\todo{Indicar as figuras onde aparecem os inputs e outputs de hw}
...
\begin{table}[h!] %http://tex.stackexchange.com/questions/50151/footnotes-in-longtable-captions. I need to fix the float.
    ...
    \begin{tabular} { m{0.05\textwidth} m{0.03\textwidth} m{0.18\textwidth} m{0.2\textwidth} m{0.35\textwidth} m{0.15\textwidth}}
        \toprule%
        ...
        \midrule%
        %CHAVE
        Int1 & \ref{fig:paineis} & Chave & Interruptor de chave & Corte geral do sistema &% &
        \begin{tabular}{l}
            Ligar \\
            Desligado \\
        \end{tabular} \\
        %Ligar SigmaD
        Int2  & \ref{fig:paineis} & Tração & Seletor de 2 posições & Liga/desliga o controlador da tração &
        \begin{tabular}{l}
            Lig.\\
            Desl.\\
        \end{tabular} \\
        %M/A
        Int3 & \ref{fig:paineis} & M/A & Seletor de 2 posições & Seleciona condução manual ou autónoma &% Ctrl\ref{hwinputs:Ctrl1} &
        \begin{tabular}{l}
            Manual \\
            Auto \\
        \end{tabular} \\
        %F/N/T
        Int4 & \ref{fig:paineis} & F/N/T & Seletor de 3 posições & Seleciona a direção da marcha, quando em modo manual &% Ctrl\ref{hwinputs:Ctrl1}; Ctrl\ref{hwinputs:Ctrl2}; Ctrl\ref{hwinputs:Ctrl3} & 
        \begin{tabular}{l}
            Frente \\
            Neutro \\
            Trás \\
        \end{tabular} \\
        %Ativar tração
        Int5 & \ref{fig:paineis} & Ativar tração & Seletor de 2 posições & Sinaliza que o veículo está pronto a andar, quando em modo manual &
        \begin{tabular}{l}
            Activar \\
            Bloq. \\
        \end{tabular} \\
        %Destravar
        Btn1 & \ref{fig:paineis} & Destravar & Botão de pressão & Ordena a destravagem do travão de emergência &
        \begin{tabular}{l}
            Pressionado \\
            Solto \\
        \end{tabular} \\
        %Desbloquear direção
        Btn2 & \ref{fig:paineis} & Desbloquear direção & Botão de pressão & Permite mover a direção em \emph{lockdown} &
        \begin{tabular}{l}
            Pressionado \\
            Solto \\
        \end{tabular} \\
        %Botão emergência
        Int6 & \ref{fig:paineis} & Interruptor de emergência & Interruptor de emergência & Envia o \ac{ROVIM} para \emph{lockdown} & 
        \begin{tabular}{l}
            Pressionado \\
            Solto \\
        \end{tabular} \\
        %Dispositivo homem morto
        & \ref{fig:paineis} & Dispositivo do homem morto & Dispositivo de homem morto & Envia o \ac{ROVIM} para \emph{lockdown} & 
        \begin{tabular}{l}
            Colocado \\
            Retirado \\
        \end{tabular} \\
        %Acelerador
        Pot1 & \ref{fig:paineis} & Acelerador & Acelerador elétrico de punho & Controla a velocidade da tração, quando em modo manual & \\
        %Travão de mão
        & \ref{fig:paineis} & Travão de mão & \emph{Manete} de travão & Trava as rodas da frente &\\
        % Esticador da correia
        & \ref{fig:atuador_dir} & Esticador da correia da direção & Rolo esticador de contacto & Permite desengatar a direção & 
        \begin{tabular}{l}
            Apertado \\
            Solto \\
        \end{tabular} \\
        %Botão dalf reiniciar
        & \ref{fig:caixa} & \emph{Reset} & Botão de pressão & Reinicia o \emph{software} do microcontrolador & 
        \begin{tabular}{l}
            Pressionado \\
            Solto \\
        \end{tabular} \\
        %Botão dalf pgm
        & \ref{fig:caixa} & BTN & Botão de pressão & Reinicia os parâmetros de origem do \emph{software}, em conjunto com o botão \emph{Reset} &
        \begin{tabular}{l}
            Pressionado \\
            Solto \\
        \end{tabular}\\
        %Alavanca ICD
        & \ref{fig:caixa} & ICD & Seletor de 2 posições & Configura o microcontrolador para arrancar a partir do \emph{debugger} &
        \begin{tabular}{l}
            ICD \\
            Oposto ICD \\
        \end{tabular} \\
        %Alavanca PGM
        & \ref{fig:caixa} & PGM & Seletor de 2 posições & Configura o microcontrolador para ser programado por porta série &
        \begin{tabular}{l}
            PGM \\
            Oposto PGM  \\
        \end{tabular}\\
...
\todo{os leds que não têm componente devem dizer qual a placa a que pertencem}
%\newcounter{ledno}
\begin{table}[htbp]
    \begin{center}
    ...
    \begin{tabular} { m{0.05\textwidth} m{0.03\textwidth} m{0.15\textwidth} m{0.1\textwidth} m{0.55\textwidth}}
        \toprule%
        \multicolumn{1}{l}{Id.} & Fig. & Elemento & Cores & Indicação\\
        \midrule%
        D2 & \ref{fig:paineis} & Indicador de destravagem & Verde & Indica que o travão está destravado, quando o botão de destravagem está pressionado\\
        D3 & \ref{fig:paineis} & Indicador de travagem & Vermelho & Indica que o travão não está destravado\\
        D1 &  \ref{fig:paineis} & Erro na tração & Vermelho & Indica um erro no controlador da tração\\
        & & Erro na tração 2 & Vermelho & Idêntico ao D1, mas encapsulado no controlador\\
        ...
        & \ref{fig:caixa} & Luz de presença controlador & Verde & Indica que o microcontrolador está ligado\\
        &  \ref{fig:caixa} & PGM & Amarelo & Indica que o microcontrolador está em modo de programação\\
        &  \ref{fig:caixa} & ICD & Amarelo & Indica que o microcontrolador está em modo de depuração\\
        & \ref{fig:caixa}  & MTR1 &
        \begin{tabular}{@{}l}
            Verde\\
            Vermelho\\
        ...
        &  \ref{fig:caixa}  & LED1 & Verde & Indica o estado do motor da tração\\
        &  \ref{fig:caixa}  & LED2 & Verde &Indica o estado do motor da direção\\
        &  \ref{fig:caixa}  & LED3 & Vermelho & Indica a existência de erros no sistema\\
...
Os indicadores MTR1, MTR2, LED1, LED2 e LED3 estão programados para fornecerem uma indicação visual básica do estado do veículo. A tabela \ref{tab:codigo_leds} apresenta os vários estados dos \acp{LED} e o seu significado.
...
\begin{table}[htbp]
    \begin{center}
        ...
        \begin{tabular}{@{} m{0.08\textwidth} m{0.15\textwidth} m{0.5\textwidth} @{}}
            ...
            \midrule
            LED1 \tnote{a} & Ligado & Motor da tração ligado e sem erros\\
                                  & Desligado & Motor da tração não está pronto a andar\\
            MTR1 \tnote{a} & Desligado & Motor da tração não está a ser acelerado\\
                                  & Verde & Motor da tração está a acelerar\\
                                  & Vermelho & Motor da tração está a desacelerar\\
            LED2 \tnote{a} \tnote{b} & Ligado & Motor da direção está estagnado\\
            ...
            MTR2 \tnote{a} \tnote{b} & Desligado & Motor da direção está desligado\\
                                  & Verde & Motor está a mover a direção para bombordo\\
                                  & Vermelho & Motor está a mover a direção para estibordo\\
            LED3 & Ligado & Bateria fraca\\
        ...
        \end{tablenotes}
        \caption{Código visual de cores dos \acsp{LED} do \acs{uC}.}
...
A interface série é uma de duas interfaces disponibilizadas pelo programa de controlo da \ac{T2D}, em conjunto com a interface \ac{I2C}.

A interface série do \ac{ROVIM} é uma extensão da interface \ac{TE} do \emph{firmware} dalf para servir os objetivos desta plataforma. A principal diferença de concepção entre as duas é que a extensão do \ac{ROVIM} foi pensada para transmitir muito mais informação ao utilizador do que a versão original. A interface original dalf foi, na medida do possível, inalterada. A sua documentação está disponível no manual do utilizador do dalf \cite{dalf_om}, secção 10, e será aqui apenas complementada para a extensão do \ac{ROVIM}.

\todo{rever}
A interface série é a interface preferencial para desenvolvimento e depuração do módulo e para condução do \ac{ROVIM} por humanos. Ao contrário da interface \ac{I2C}, permite comunicar assincronamente com o utilizador e pode ser usada para controlar a \ac{T2D} independentemente dos outros módulos do \ac{ROVIM}, sendo que de momento, apenas esta interface disponibiliza todas as funcionalidades do programa. Esta é estabelecida através de uma ligação RS232 em modo \ac{ASCII}, configurada para 57600 baud/s, 8 bit, sem paridade, e com 1 bit de paragem, que pode ser usada com recurso a uma aplicação de emulador de terminal num \ac{PC}. Os parâmetros estão formatados em formato hexadecimal.
...
A tabela \ref{tab:comandos_rovim} apresenta resumidamente os comandos específicos da extensão da interface para o \ac{ROVIM}, assim como os comandos da interface original dalf mais úteis para esta aplicação, servindo como listagem rápida dos comandos disponíveis.
...
Alguns comandos da interface série exigem a identificação do motor sobre o qual atuar. Existem dois motores registados no \emph{software}: o motor da tração e o motor da direção. O motor da tração é identificado pelo número 1, enquanto que o da direção é o número 2. Quando a identificação do motor é facultativa, o comando atua sobre ambos se esta não for especificada.
...
Este comando envia o \ac{ROVIM} para o estado de \emph{lockdown}. O estado de \emph{lockdown} é descrito em \ref{sec:programa}.
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G10} & Entra em modo de segurança\\
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G11} & Sai do modo de segurança
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G12 01} & Pára o motor da tração\\
        \textsf{G12} & Pára ambos os motores\\
\end{tabular}
\end{center}
...
Este comando permite aceder aos pinos programáveis de entrada e saída do microcontrolador. É usado principalmente pelas funções internas do \emph{software} do \ac{ROVIM}, mas está acessível aos utilizadores.
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} @{\qquad} m{0.6\textwidth} @l}
        \textsf{G13 01 0D} & Trava o travão de emergência\\
        \textsf{G13 01 0E} & Erro - não é possível definir valor de pinos de entrada\\
        \textsf{G13 02 09} & Liga a marcha trás em modo autónomo\\
        \textsf{G13 03 0F} & Alterna a ativação do travão de mão\\
        \textsf{G13 04 0E} & Retorna \num{1} se o interruptor de emergência estiver pressionado, ou \num{0}, se estiver solto\\
        \textsf{G13 04 09} & Retorna \num{0} se a marcha atrás estiver ligada em modo autónomo, \num{1} em caso contrário\\
\end{tabular}
\end{center}
...
Este comando controla o acelerador do motor da tração do \ac{ROVIM}, em modo autónomo de condução, de modo semelhante ao acelerador de punho.
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G14 0} & Coloca o acelerador a \SI{0}{\%}\\
        \textsf{G14 32} & Coloca o acelerador a \SI{50}{\%}\\
\end{tabular}
\end{center}
...
Este comando controla a recuperação de energia do motor da tração do \ac{ROVIM}, que funciona como mecanismo de travagem controlada em modo autónomo de condução. É usado para diminuir a velocidade do veículo em condições normais de condução. Para manter o veículo imóvel ou travagens de emergência devem ser usados os comandos apropriados, respetivamente \textsf{G10} e \textsf{G16}.

\textbf{Exemplo:}
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G15 0} & Desativa o desacelerador\\
        \textsf{G15 64} & Liga o desacelerador ao máximo\\
\end{tabular}
\end{center}
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G16 0 20} & Coloca o veículo a andar para a frente a cerca de \SI{3.2}{Km/h}\\
        \textsf{G14 40} & Ajusta o acelerador para \SI{64}{\%}\\
        \textsf{G16 1 4} & Erro - não é permitida uma velocidade tão baixa\\
        \textsf{G16 1 10} & Inverte a marcha a \SI{1}{Km/h}\\
        \textsf{G16 2} & Coloca o veículo em ponto morto\\
        \textsf{G16 3 0} & Pára e imobiliza o veículo\\
        \textsf{G16} & Pára e imobiliza o veículo\\
\end{tabular}
\end{center}
...
O ângulo de viragem é definido em \ref{sec:programa}, e varia entre \ang{0} (tudo a estibordo) e \ang{86} (tudo a bombordo), sendo \ang{43} o ponto central, em que a direção está alinhada (a moto segue em frente).
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G16 0 A} & Coloca o veículo a andar a cerca de \SI{1}{Km/h}\\
        \textsf{G17 2B} & Alinha  a direção\\
        \textsf{G17 56} & Vira tudo a bombordo\\
        \textsf{G16} & Pára o veículo\\
        \textsf{G17} & Pára o motor da direção\\
        \textsf{G17 2B} & Erro - o \ac{ROVIM} não está em movimento\\
\end{tabular}
\end{center}
...
Apresenta o nível de verbosidade no ecrã. A secção \ref{sec:programa} descreve em maior detalhe os níveis de verbosidade disponíveis.

\subparagraph*{2=Definir nível de verbosidade} \mbox{}

Define o novo nível de verbosidade. O nível de verbosidade a definir é passado no argumento \textbf{D1}. A secção \ref{sec:programa} descreve em maior detalhe os níveis de verbosidade disponíveis.
...
\begin{center}
    \begin{tabular}{@{} m{0.2\textwidth} m{0.6\textwidth}}
        \textsf{G18 01} & Mostra o nível de verbosidade\\
        \textsf{G18 02 0F} & Repõe o nível de verbosidade predefinido\\
        \textsf{G18 03} & Alterna o modo de repetição da tarefa de monitorização do sistema\\
        \textsf{G18 04} & Corre a tarefa de monitorização\\
        \textsf{G18 05 05 01 14} & Estabelece o acelerador a \SI{20}{\%} e arranca a depuração da tarefa de geração de \ac{PWM} durante 5 períodos do sinal\\
        \textsf{G18 06 01} & Reinicia a sistema\\
        \textsf{G18 07} & Alterna a limitação à execução dos comandos interditos em \emph{lockdown} \\
        \textsf{G18 09} & Mostra informação sobre os erros atuais\\
\end{tabular}
\end{center}
...
O comando de ajuda apresenta a seguinte mensagem:
 \verbatiminput{recursos/mensagem_ajuda.txt}
...
\caption{Fluxograma do processo de ligar.}
\label{flux:ligar}
\end{figure}
...
Em condições normais estas verificações serão sempre passadas com sucesso, mas é importante estar ciente delas, especialmente após longos períodos de inatividade ou após utilização do veículo por outros utilizadores.

\subsubsection{Configuração inicial dos seletores}
\label{sssec:configurar}

A fim de assegurar que o arranque se processa sem erros, o utilizador deve garantir que o interruptor de emergência e os seletores do painel de controlo do \ac{ROVIM} se encontram no seguinte estado:
...
\begin{description}[noitemsep] % sets no itemsep for just this list
    ...
    \item[Interruptor de emergência] Solto.
\end{description}
...
\caption{Fluxograma do processo de desligar.}
\label{flux:desligar}
\end{figure}
...
\begin{itemize}
    \item Ativando o interruptor de emergência;
    \item Retirando o dispositivo do homem morto;
    \item Enviando um comando pela porta série (\textsf{G10}).
\end{itemize}

O estado de \emph{lockdown} é descrito com mais detalhe em \ref{sec:programa}.
...
\caption{Fluxograma do processo de sair do estado de \emph{lockdown}.}
\label{flux:sair_lockdown}
\end{figure}

Nas situações em que aconteça um erro e o programa entre neste modo de segurança, é necessário primeiramente recuperar do erro. Só quando o programa não detetar erros é que sai com sucesso deste estado.
...
\caption{Fluxograma do processo de comutação para modo autónomo de condução.}
...
\caption{Fluxograma do processo de comutação para modo manual de condução.}
...
\subsubsection{Arranque em modo manual} \mbox{}
...
O acelerador funciona de modo diferente para os dois modos de condução. Em modo manual é um tradicional acelerador analógico de punho, enquanto que em modo autónomo este é controlado pelo comando \textsf{G14}.
...
Em modo manual esta funcionalidade é ativada em ponto morto, ao bloquear a tração com recurso ao seletor "Ativar tração". Em modo autónomo, o comando \textsf{G16} permite selecionar esta funcionalidade.
...
O arranque consiste em ligar a moto (ver \ref{ssec:ligar}), sair de \emph{lockdown} (ver \ref{ssec:sair_lockdown}), selecionar o modo de condução (ver \ref{ssec:alternar_modos}) e selecionar a marcha (ver \ref{ssec:selecionar_marcha}). O \ac{ROVIM} está então pronto a ser conduzido.
...
\begin{itemize}
    ...
    \item Códigos de erro do controlador da tração. O controlador da tração tem a capacidade de detetar erros. Em caso de erro, este faz piscar os \acp{LED} de erro na tração com um padrão que varia consoante o seu código. Os códigos dos erros do controlador, e as suas causas e soluções podem ser consultadas no manual técnico, \cite{datasheet_PMT835M};
...
\begin{enumerate}
    \item A velocidades baixas (menos de \SI{0.5}{Km/h}) o sensor de velocidade não tem exatidão;
